...
import com.flipit.ui.dialogs.ReportDialog;
...
public class BrowsePanel extends JPanel {
    ...
    private JButton publicDecksTab;
    private JButton disabledDecksTab;
    ...
    private boolean isDisabledView = false;
    ...
    private long cacheTimestampPublic = 0;
    private long cacheTimestampDisabled = 0;
    ...
    public BrowsePanel(MainPanel mainPanel, User user) {
        ...
        publicDecksTab.addActionListener(e -> {
            isDisabledView = false;
            updateTabs();
            loadDecks();
        });

        disabledDecksTab.addActionListener(e -> {
            isDisabledView = true;
            updateTabs();
            loadDecks();
        });
    ...
    public void clearCache() {
        cachedPublicDecks = null;
        cachedDisabledDecks = null;
    }

    private void updateTabs() {
        styleTab(publicDecksTab, !isDisabledView);
        styleTab(disabledDecksTab, isDisabledView);
    }
    ...
    private void styleAll() {
        ...
        searchField.setText("Search by title, creator, or tags...");
        searchField.setForeground(Color.decode("#8792a8"));
        searchField.addFocusListener(new FocusAdapter() {
            public void focusGained(FocusEvent evt) {
                if (searchField.getText().equals("Search by title, creator, or tags...")) {
        ...
        sortComboBox.setModel(new DefaultComboBoxModel<>(new String[]{
                "Most Popular", "Newest", "Oldest", "Most Cards", "Fewest Cards", "Title (A-Z)", "Title (Z-A)"
        }));
    ...
    private void loadDecks() {
        ...
        if (q.equals("search by title, creator, or tags...")) q = "";
        final String searchQuery = q;
        final int sortIdx = sortComboBox.getSelectedIndex();
        final boolean currentDisabledView = isDisabledView;
        ...
        deckGridPanel.setBorder(BorderFactory.createEmptyBorder(
                ImageUtil.scale(15), ImageUtil.scale(10), ImageUtil.scale(15), ImageUtil.scale(10)
        ));
        for (int i = 0; i < 4; i++) {
        ...
        deckLoaderWorker = new SwingWorker<>() {
            ...
            protected List<DeckRenderData> doInBackground() throws Exception {
                List<Deck> decks;
                if (currentDisabledView && (user.isModerator() || user.isAdmin())) {
                    decks = deckDAO.getAllSystemDecks();
                    decks.removeIf(d -> !d.isDisabled());
                } else {
                    decks = deckDAO.getPublicDecks();
                }
                ...
                for (Deck d : decks) {
                    boolean matchesSearch = searchQuery.isEmpty() ||
                            d.getTitle().toLowerCase().contains(searchQuery) ||
                            (d.getCreatorName() != null && d.getCreatorName().toLowerCase().contains(searchQuery));
    ...
    private void renderList(List<DeckRenderData> listToRender, int sortIdx) {
        listToRender.sort((d1, d2) -> {
            ...
            if (sortIdx == 0) {
                int pop1 = d1.saves + d1.views;
                int pop2 = d2.saves + d2.views;
                if (pop1 != pop2) return Integer.compare(pop2, pop1);
                return t2.compareTo(t1);
            }
            if (sortIdx == 2) return t1.compareTo(t2);
            if (sortIdx == 3) return Integer.compare(d2.deck.getCardCount(), d1.deck.getCardCount());
            if (sortIdx == 4) return Integer.compare(d1.deck.getCardCount(), d2.deck.getCardCount());
            if (sortIdx == 5) return d1.deck.getTitle().compareToIgnoreCase(d2.deck.getTitle());
            if (sortIdx == 6) return d2.deck.getTitle().compareToIgnoreCase(d1.deck.getTitle());
        ...
        if (listToRender.isEmpty()) {
            deckGridPanel.setLayout(new GridBagLayout());
            deckGridPanel.add(buildEmptyState());
        } else {
            deckGridPanel.setLayout(new GridBagLayout());
            ...
            for (DeckRenderData data : listToRender) {
                ...
                deckGridPanel.add(buildBrowseCard(data), c);
    ...
    private JPanel buildEmptyState() {
        ...
        JLabel msg = new JLabel(isDisabledView ? "No disabled decks found." : "No public decks found.");
        ...
        JLabel sub = new JLabel(isDisabledView ? "There are no disabled decks." : "Looks like no one has shared anything matching that search!");
    ...
    private JPanel buildBrowseCard(DeckRenderData data) {
        Deck deck = data.deck;
        ...
        Timestamp displayDate = deck.getPublishedAt() != null ? deck.getPublishedAt() : deck.getCreatedAt();
        JLabel dateLbl = new JLabel(getTimeAgo(displayDate));
        ...
        JPopupMenu optMenu = buildOptionsMenu(deck, baseFont);
        ...
        JPanel savesStatPanel = createStatLabel("SAVE", data.saves + " saves", Color.decode("#64748b"), sf);
        JLabel savesLabel = (JLabel) savesStatPanel.getComponent(1);
        statsRow.add(savesStatPanel);
        ...
        if (!user.isAdmin()) {
            ...
            if (!isOwnDeck) {
                final boolean[] savedState = {data.isSaved};
                JButton saveBtn = saveButton(savedState, isOwnDeck);

                saveBtn.addActionListener(e -> {
                    boolean currentlySaved = savedState[0];
                    savedState[0] = !currentlySaved;
                    saveBtn.repaint();

                    data.isSaved = savedState[0];
                    data.saves += currentlySaved ? -1 : 1;
                    savesLabel.setText(data.saves + " saves");

                    new SwingWorker<Void, Void>() {
                        @Override
                        protected Void doInBackground() {
                            if (currentlySaved) deckDAO.unsaveDeck(user.getId(), deck.getId());
                            else deckDAO.saveDeck(user.getId(), deck.getId());
                            return null;
                        }
                ...
                JButton saveBtn = saveButton(new boolean[]{false}, true);
                actionRow.add(saveBtn, BorderLayout.EAST);
            }
    ...
    private JButton saveButton(boolean[] savedState, boolean isOwnDeck) {
        ...
        if (!isOwnDeck) btn.setCursor(Cursor.getPredefinedCursor(Cursor.HAND_CURSOR));
        return btn;
    }
    ...
    private JPopupMenu buildOptionsMenu(Deck deck, Font baseFont) {
        ...
        if (deck.getUserId() == user.getId() || user.isModerator() || user.isAdmin()) {
                ...
                if (fc.showSaveDialog(this) == JFileChooser.APPROVE_OPTION) {
                    ...
                    new SwingWorker<Void, Void>() {
                        ...
                        protected void done() {
                            try {
                                get();
                                new SuccessDialog(SwingUtilities.getWindowAncestor(BrowsePanel.this), "Success", "File downloaded successfully!").setVisible(true);
                            } catch (Exception ex) {
                                ex.printStackTrace();
                                new InfoDialog(SwingUtilities.getWindowAncestor(BrowsePanel.this), "Error", "Error saving file. " + ex.getMessage()).setVisible(true);
                            }
                        }
                    }.execute();
                }
        ...
        if (deck.getUserId() == user.getId() || user.isModerator() || user.isAdmin()) {
            ...
            JMenuItem toggleState;
            if ((user.isModerator() || user.isAdmin()) && deck.getUserId() != user.getId()) {
                toggleState = menuItem(deck.isDisabled() ? "Enable Deck" : "Disable Deck", Color.decode("#f59e0b"), false, baseFont);
                toggleState.addActionListener(e -> {
                    String actionTxt = deck.isDisabled() ? "enable this deck?" : "disable this deck?";
                ...
                toggleState = menuItem(deck.isPublic() ? "Make Private" : "Make Public", Color.decode("#3b5bdb"), false, baseFont);
                toggleState.addActionListener(e -> {
                    if (deck.isDisabled()) {
                        new InfoDialog(SwingUtilities.getWindowAncestor(this), "Disabled", "This deck is disabled by a moderator and cannot be made public.").setVisible(true);
                        return;
                    }
                    String actionTxt = deck.isPublic() ? "make this deck private?" : "make this deck public?";
                    WarningDialog dialog = new WarningDialog(SwingUtilities.getWindowAncestor(this),
                            "Confirm Visibility",
                            "Are you sure you want to " + actionTxt,
                            "Confirm");
    ...
    private void renameDeck(Deck deck) {
        ...
        if (dialog.isApproved()) {
            ...
            if (!name.isBlank()) {
                ...
                if (!finalName.equalsIgnoreCase(deck.getTitle())) {
                    if (deck.isPublic() && deckDAO.isPublicTitleTaken(finalName, deck.getId())) {
                        new InfoDialog(parentWindow, "Conflict", "A public deck with this name already exists.").setVisible(true);
                        return;
                    }
                    if (deckDAO.isUserTitleTaken(user.getId(), finalName, deck.getId())) {
                ...
                new SwingWorker<Void, Void>() {
                    @Override
                    protected Void doInBackground() {
                        deckDAO.updateDeck(deck.getId(), finalName, deck.getDescription(), deck.isPublic());
                        return null;
                    }
...
// DO NOT EDIT OR ADD ANY CODE HERE!
    ...
    private void $$$setupUI$$$() {
        ...
        publicDecksTab = new JButton();
        publicDecksTab.setText("Public Decks");
        tabPanel.add(publicDecksTab);
        disabledDecksTab = new JButton();
        disabledDecksTab.setText("Disabled Decks");
        tabPanel.add(disabledDecksTab);
