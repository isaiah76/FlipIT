...
CREATE OR REPLACE VIEW view_deck_count AS
SELECT
    decks.id,
    decks.user_id,
    decks.file_id,
    ...
    uploaded_files.file_name AS source_file_name,
    COUNT(DISTINCT cards.id) AS card_count,
    GROUP_CONCAT(DISTINCT deck_tags.tag_name SEPARATOR ',') AS tags
FROM decks
...
GROUP BY
    decks.id, decks.user_id, decks.file_id, decks.title, decks.description,
    decks.is_public, decks.is_disabled, decks.created_at, decks.published_at,
    users.username, uploaded_files.file_name;
